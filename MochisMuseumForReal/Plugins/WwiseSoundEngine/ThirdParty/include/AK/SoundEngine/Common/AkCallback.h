...
struct AkMixerInputInfo
{
	AkConnectionType       eConnectionType;               ///< Nature of the connection between this input and the mixer.
	AkReal32               fCenterPerc;                   ///< Center percentage of this input, between 0 and 1.
	AkSpeakerPanningType   eSpeakerPanningType;           ///< Specifies type of panning logic when object is not 3D spatialized. Applicable only when eSpatializationMode is AK_SpatializationMode_None.
	Ak3DSpatializationMode eSpatializationMode;           ///< The 3D spatialization mode used by this input. Applicable only when the input has listener relative routing (see bHasListenerRelativeRouting).
	Ak3DPositionType       e3DPositionType;               ///< Get whether the emitter position is defined by the game alone (AK_3DPositionType_Emitter), or if it is further automated. Applicable only when the input has listener relative routing (see bHasListenerRelativeRouting).
...
struct AkSpeakerVolumeMatrixCallbackInfo : public AkEventCallbackInfo
{
	...
	AkMixerInputInfo inputInfo;                 ///< Information about the current voice/bus about to be mixed into the output bus with specified base volume and volume matrix.
};
